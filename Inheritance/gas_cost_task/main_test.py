...
def test(vehicle, distance, fuel_price, expected_cost):
    try:
        vehicle_type = vehicle.__class__.__name__
        print("---------------------------------")
        print(
            f"Testing {vehicle_type}: Max Speed {vehicle.max_speed} kph, Fuel Efficiency {vehicle.efficiency} km/l"
        )
        print(f"Distance: {distance} km, Fuel Price: {fuel_price} per liter")
        print(f"Expected Trip Cost: {expected_cost}")
        actual_cost = vehicle.get_trip_cost(distance, fuel_price)
